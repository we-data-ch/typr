...
**Avancement (2026-07-18) : Phase 1 (S1, S2, S3) faite. Phase 2 (M1-M5) faite. Phase 3 (G1, G2, G3) faite. Phase 4 (O1, O2, C1, C2) faite ; A1/A2 vérifiés non-bugs. Phase 5 (I1, A3, D1) faite. Phase 6 (harnais préventif) faite. Phase 7 (C3, C4, I2, G4, G5, G6) faite. Phase 8 (bug `ConstructorCall` générique découvert en vérifiant G6) faite. Phase 9 (U1, I3, T1) faite — audit clos.** Voir tests `edge_*` dans `type_checking::tests` (mod.rs) pour Phases 1/4/5/7/9(I3), `edge_*` dans `match_expression::tests` pour Phase 2, les tests dans `unification::tests` / `unification_map::tests` / `let_expression::tests` pour Phase 3, `type_comparison::tests` pour Phase 6 + G4/G5 (Phase 7), `test_implicit_mutate_dollar_target_fails`/`test_implicit_mutate_array_indexing_target_fails` (`parsing/mod.rs`) pour A1, `edge_p8_*` dans `constructor_call::tests` pour Phase 8, `edge_u1_*` dans `constructor_call::tests` pour Phase 9 (U1), `edge_i3_*` dans `type_checking::tests` pour Phase 9 (I3), et `test_let_tuple_*_binding_is_arity_error` dans `parsing::tests` pour Phase 9 (T1). Détail dans les tables §1/§2/§3/§4/§5/§6/§8 ci-dessus. Reste seul G7 (reporté, chevauche d'autres chantiers en mémoire).
...
| ID | Localisation | Problème | Sévérité |
|---|---|---|---|
| U1 | ~~`type_checking/mod.rs:2321-2329` (`Lang::UnionConstructor`) + `constructor_call.rs`~~ | ✅ **FIXÉ (2026-07-18, Phase 9)** : `TagName:{ field = val }` / `Union.Variant:{ field = val }` (syntaxe qualifiée documentée en §7 de `syntaxe.md`) ne validait RIEN et cassait systématiquement à l'exécution R. **Piste 1 retenue** (rendre le cas faux impossible à écrire plutôt que de patcher le runtime) : le bras de typage `Lang::UnionConstructor` (`mod.rs`) résout maintenant vraiment `union_name`/`variant_name` — `TypeError::AliasNotFound` si `union_name` n'existe pas, `TypeError::UnknownUnionVariant` si `variant_name` n'est membre d'aucune variante — et distingue deux formes de membre : un **alias record utilisé tel quel comme membre de l'union** (ex. `Rgb` dans `.Red | Rgb`) délègue à `constructor_call::constructor_call` (validation de champs complète, réutilisée telle quelle) ; un **vrai tag** (`.Variant(...)`, scalaire ou à corps record, peu importe) rejette systématiquement `:{...}` et même la forme nue non vide via la nouvelle `TypeError::TagFieldConstructorNotSupported` — seule une référence nue à un tag *sans charge utile* (`Color.Red`) reste valide, car c'est la seule forme dont le R généré (`Variant <- function(x) {...}`, un seul paramètre positionnel) correspond réellement à la syntaxe. La forme non qualifiée (`Circle:{...}`, `Lang::ConstructorCall`) reçoit le même filet dans `constructor_call.rs` via un nouveau helper `find_tag_owner` qui cherche `type_name` parmi les tags de toute union en portée avant de retomber sur `Any` silencieux. Vérifié en pipeline réel (`typr new`+`check`+`run`) : `Shape.Circle:{r=3.0}` et `Circle:{r=3.0}` sont désormais rejetés à la compilation avec un message clair et un `help` pointant vers `.Circle(value)` ; `.Circle(3.0)` + `match` continue de fonctionner et d'imprimer la bonne valeur ; `Color.Rgb:{r=10,g=20,b=30}` (alias record en membre d'union) continue de fonctionner et d'imprimer la bonne valeur. 7 tests dans `constructor_call::tests` (`edge_u1_*`). `cargo test --workspace` (613 tests typr-core lib, +7 vs Phase 8 — guard `guard_any_type_call_count_is_tracked` bump 16→19, baseline expliquée dans le commentaire, les 3 nouveaux `any_type()` étant chacun appairés à une erreur poussée) + `fmt --check` + `clippy` : zéro régression, zéro nouveau warning sur les fichiers touchés. `syntaxe.md` §7 mis à jour (l'exemple fabriqué `.Circle(num)` + `:{r=3.0}` remplacé par les deux cas réels : tag nu sans charge utile, et alias record en membre d'union). | ~~**Haute**~~ |
| I3 | ~~`type_checking/mod.rs:1702-1753` (`Lang::ArrayIndexing`, branche générale) via `Type::to_array2` (`components/type/mod.rs:927-944`)~~ | ✅ **FIXÉ (2026-07-18, Phase 9)** : indexer un `Array`/`Vec` avec un index scalaire (`a[1]`) typait systématiquement en `[1, int]`, quel que soit le vrai type d'élément (`Type::to_array2` reconstruisait le type résultat à partir des *valeurs d'index*, avec un `int` codé en dur comme dimension interne, au lieu de lire le vrai type de base de `typ1`). Corrigé en supprimant `to_array2` (plus aucun appelant) et en ajoutant `Type::from_linear`, l'inverse exact de `Type::linearize` (déjà utilisé juste au-dessus pour `args_target`) : reconstruit un `Type::Vec` imbriqué à partir de `[dim1, .., dimN, base]` plutôt que d'inventer une forme. Le site d'appel (`Lang::ArrayIndexing`) distingue maintenant explicitement les deux cas que l'audit avait identifiés : si l'index fourni couvre *toutes* les dimensions réelles de `typ1` (`args_index.len() >= args_target.len() - 1`), le résultat est directement `args_target.last()` (le vrai type élément) ; sinon (indexation partielle, sous-tableau) le résultat est `Type::from_linear` appliqué aux dimensions non consommées (`args_target[args_index.len()..]`), qui se termine déjà par le vrai type de base — plus de type inventé dans aucun des deux cas. Vérifié en pipeline réel (`typr new`+`check`+`run`) : `let a <- ["un","deux","trois"]; let f <- fn(x: char): char { x }; f(a[1]) |> print();` type-check désormais correctement (auparavant rejeté avec le faux `Function f<[1, int]> not defined in this scope` décrit ci-dessous) et exécute réellement en R, imprimant `un`. 3 tests dans `type_checking::tests` (`edge_i3_*`) : type réel d'un `a[1]` sur tableau `char` (`Char`, pas `[1, int]`), non-régression sur tableau `int` (`Integer`, pas `int` accidentel), et le cas concret ci-dessus (élément passé à une fonction typée `char`). `cargo test --workspace` (609 tests typr-core lib, stable +3 vs baseline — l'écart apparent avec le « 613 » de Phase 8/U1 vient de fichiers de tests non commités entre-temps, pas d'une régression) + `fmt --check` + `clippy` : zéro régression, zéro nouveau warning sur les fichiers touchés. **Trou de couverture comblé** : zéro test existant ne couvrait le *type* d'un accès scalaire sur `Array`/`Vec` avant ça (seul `test_tuple_scalar_indexing` existait, pour les tuples, et les tests I2 ne vérifiaient que l'absence de panic/l'erreur sur index non-`int`, jamais le type retourné pour un index valide). | ~~**Haute**~~ |
| T1 | ~~`parsing/mod.rs` (désucrage de `let :{a, b, ...} <- tuple_expr;`, cf. §2 `syntaxe.md`)~~ | ✅ **FIXÉ (2026-07-18, Phase 9)** : `let_tuple_exp` (`parsing/mod.rs`) compare maintenant `elements.len()` (nombre de variables du pattern `:{...}`) au nombre d'éléments réel dès que le membre droit du `<-` est syntaxiquement un littéral tuple (`Lang::Tuple`) — le seul cas où l'arité est connue sans type-checking, comme décidé lors du catalogage initial. Mismatch (dans les deux sens) → nouvelle erreur `SyntaxError::TupleDestructureArityMismatch { expected, found, help_data }`, poussée via `push_parse_error` avec la position du pattern `:{...}` dans le fichier utilisateur (pas `std.ty`). Classée **fatale** (pas un simple warning) dans `report_syntax_errors` (`typr-cli/src/project.rs`) aux côtés de `SingleLetterTypeName`/`KeywordRecordPositionalElements` : le sous-binding perdait silencieusement des données sans le moindre diagnostic, exactement la catégorie que ce groupe fatal existe pour bloquer. Une source dynamique (appel de fonction, variable, ...) n'est **pas** vérifiée ici — hors scope (nécessiterait le type inféré, pas juste la syntaxe) ; le chemin d'indexation générique existant reste le seul filet pour ce cas, inchangé. Vérifié en pipeline réel (`typr new`+`check`+`build`+`run`) : `let :{a,b} <- :{1,2,3};` et `let :{a,b,c,d} <- :{1,2,3};` échouent désormais à la compilation avec message clair et position source ; `let :{a,b,c} <- :{1,2,3};` continue de type-checker, transpiler et s'exécuter correctement (imprime `1 2 3`). 6 tests dans `parsing::tests` (`test_let_tuple_under_binding_is_arity_error`, `test_let_tuple_over_binding_is_arity_error`, `test_let_tuple_matching_arity_no_error`, `test_let_tuple_wildcard_counts_toward_arity` — `_` occupe quand même un slot d'arité —, `test_let_tuple_dynamic_source_no_arity_check`, `test_let_tuple_arity_mismatch_full_parse_has_errors`). `cargo test --workspace` (615 tests typr-core lib, +6, zéro régression) + `fmt --check` + `clippy` : zéro régression, zéro nouveau warning sur les fichiers touchés. | ~~Moyenne~~ |
...
### Phase 9 — U1, I3 et T1 faits (audit clos, hors G7 reporté)

**U1 fait (2026-07-18)** — piste 1 choisie par l'utilisateur, précisée en cours de route : plutôt
que d'accepter `:{...}` seulement quand le tag encapsule un `list{...}` (piste initialement
esquissée), l'investigation a montré qu'un `list{...}` embarqué dans un tag ne suffit **pas** à
rendre `:{...}` valide — le R généré pour *tout* tag (`Variant <- function(x) {...}`) prend une
seule charge utile positionnelle quelle que soit sa forme, jamais des champs nommés séparés. La
règle retenue est donc : `:{...}` (et même la forme nue non vide) n'est légitime que quand
`Variant` est un **alias record utilisé tel quel comme membre de l'union** (pas un `.Variant(...)`
tag) — auquel cas il a son propre constructeur à champs nommés, réutilisé sans duplication de
logique. Toute autre combinaison est maintenant un `TypeError` explicite (`TagFieldConstructorNotSupported`
pour le mismatch champs/tag, `UnknownUnionVariant`/`AliasNotFound` pour un nom inconnu) plutôt qu'un
`Any` silencieux qui plantait ensuite au runtime R. Détail complet dans la table ci-dessus (§8, ligne
U1) et les tests `edge_u1_*` (`constructor_call.rs`).

**I3 fait (2026-07-18)** — `Type::to_array2` supprimée (plus aucun appelant après le fix) et
remplacée par `Type::from_linear`, l'inverse exact de `Type::linearize` : la branche générale de
`Lang::ArrayIndexing` distingue maintenant "l'index couvre toutes les dimensions → retourner
l'élément réel (`args_target.last()`)" de "l'index n'en couvre qu'une partie → retourner un
sous-tableau sur les dimensions non consommées (`Type::from_linear(args_target[n..])`)". Détail
complet dans la table ci-dessus (§8, ligne I3) et les tests `edge_i3_*` (`type_checking/mod.rs`).

**T1 fait (2026-07-18)** — check d'arité ajouté au désucrage `let_tuple_exp` (`parsing/mod.rs`),
limité au cas où le membre droit est syntaxiquement un littéral tuple (`Lang::Tuple`), nouvelle
`SyntaxError::TupleDestructureArityMismatch` classée fatale dans `report_syntax_errors`. Détail
complet dans la table ci-dessus (§8, ligne T1) et les tests `test_let_tuple_*_binding_is_arity_error`
(`parsing/mod.rs`). Avec T1 fait, l'audit de la section 8 (U1, I3, T1) est clos ; seul G7 (§3-7,
reporté, chevauche `project_interface_dispatch_hybrid`/`project_record_interface_intersection`)
reste ouvert dans tout l'audit `audit_type_checking.md`.
