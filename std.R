...
fold <- function(vec, ...) UseMethod("fold")
...
# `cat` is a plain (non-S3-generic) base-R function; the `.default` is both the
# UseMethod fallback and where TypR's own boxing is peeled off each argument so
# `cat("x:", x)` prints bare values, not `attr(,"class")` noise.
cat.default <- function(...) {
  args <- lapply(list(...), to_native)
  do.call(base::cat, args)
  invisible(NULL)
}

# The string/file/system helpers below (default.ty, file.ty, system.ty) are
# plain, non-S3-generic base-R functions (see r_name_lint's Phase C database,
# soundness_transpilation.md). Without a `.default` fallback, the UseMethod
# stub generic_functions.R emits for each of them clobbers the base function
# with zero method to dispatch to — the exact `nlevels` bug shape, caught for
# all of these at once when the Phase C lint was first run against a fresh
# project. Argument order below mirrors each `.ty` signature (see
# configs/std/default.ty / file.ty / system.ty), which already matches base
# R's own argument order in every case.
nchar.default      <- function(a, ...) base::nchar(a, ...)
sd.default         <- function(a, ...) base::sd(a, ...)
substr.default     <- function(a, b, e, ...) base::substr(a, b, e)
sub.default        <- function(a, b, c, ...) base::sub(a, b, c, ...)
gsub.default       <- function(a, b, c, ...) base::gsub(a, b, c, ...)
strsplit.default   <- function(s, d, ...) base::strsplit(s, d, ...)[[1]]
tolower.default    <- function(a, ...) base::tolower(a)
toupper.default    <- function(a, ...) base::toupper(a)
startsWith.default <- function(a, b, ...) base::startsWith(a, b)
endsWith.default   <- function(a, b, ...) base::endsWith(a, b)
grepl.default      <- function(a, b, ...) base::grepl(a, b, ...)
setwd.default      <- function(a, ...) base::setwd(a)
unlink.default     <- function(a, ...) base::unlink(a, ...) == 0L
system2.default <- function(command, args, stdout, stderr, stdin, ...) {
  base::system2(command, args = args, stdout = stdout, stderr = stderr, stdin = stdin, ...)
}

# `version` is only ever meaningfully defined for `State<T>` (see
# `version.State` below) — there is no sensible base-R fallback, so the
# `.default` gives a clear, localized error instead of the generic "no
# applicable method" UseMethod message.
version.default <- function(x, ...) {
  stop(sprintf(
    "version() is only defined for State<T> values, got class <%s>",
...
fold.typed_vec <- function(vec, init, f, ...) {
