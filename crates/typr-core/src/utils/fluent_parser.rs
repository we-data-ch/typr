...
impl FluentParser {
    ...
    pub fn parse_next(self) -> Self {
        match self.clone().next_raw_code() {
            Some((line, rest)) => match parse2((&line[..]).into()) {
    ...
    pub fn parse_all_lines(self) -> Self {
        self.clone()
            .raw_code
            .iter()
            .fold(self, |acc, x| match parse2(x[..].into()) {
    ...
    pub fn transpile_next(self) -> Self {
        match self.clone().next_new_code() {
            Some((code, rest)) => {
                ...
                let res = rest.set_context(new_context).push_r_code(r_code);
    ...
    pub fn parse_type_transpile_next(self) -> Self {
        self.parse_next().type_next().transpile_next()
    }
    ...
    pub fn check_parsing(self, s: &str) -> Vector<Lang> {
        self.push(s).parse_next().get_code()
    }

    pub fn check_typing(self, s: &str) -> Type {
        self.push(s).parse_type_next().get_last_type()
    }

    pub fn check_transpiling(self, s: &str) -> Vector<String> {
        self.push(s).parse_type_transpile_next().get_r_code()
    }
}
