...
pub fn dollar_access(context: &Context, expr: &Lang, e1: &Lang, e2: &Lang, hd: &HelpData) -> TypeContext {
    let op = match expr {
    ...
    match (ty1.reduce(context), e2.clone(), &op) {
        ...
        (reduced_ty, Lang::Variable { name, help_data: h, .. }, _)
        ...
        (Type::Module(fields, _, _), Lang::Variable { name, help_data: h, .. }, _) => {
        ...
        (Type::Module(fields, _, _), Lang::FunctionApp { identifier: exp, .. }, _) => {
                ...
                Some(var) => {
                        ...
                        .find(|arg_typ2| arg_typ2.get_argument_str() == var.get_name())
                    {
                        Some(arg_typ) => match FunctionType::try_from(arg_typ.get_type()) {
                            Ok(ft) => TypeContext::new(ft.get_return_type(), expr.clone(), context.clone())
                                .with_errors(errors),
                            Err(_) => {
                                errors.push(TypRError::Type(TypeError::WrongExpression(exp.get_help_data())));
        ...
        (Type::Tuple(vals, _), Lang::Integer { value: i, help_data: h }, _) => match vals.get((i - 1) as usize) {
            ...
            Some(var) => {
                    ...
                    .find(|arg_typ2| arg_typ2.get_argument_str() == var.get_name())
                {
                    Some(arg_typ) => {
                        ...
                        TypeContext::new(tc.value, expr.clone(), context.clone()).with_errors(errors)
                    }
                    None => {
        ...
        (Type::UnknownFunction(h), Lang::FunctionApp { .. }, _) => {
            TypeContext::new(Type::UnknownFunction(h), expr.clone(), context.clone()).with_errors(errors)
        }
        ...
        (
            ...
            Type::Record(fields, _) => {
                match field_access::find_field_or_push_error(fields, &name, &ty1, vh.clone(), &mut errors) {
                    Some(field_ty) => {
                    ...
                    None => TypeContext::new(builder::any_type(), expr.clone(), context.clone()).with_errors(errors),
                }
            }
            _ => {
