...
use crate::engine::{parse_code, parse_code_from_str, write_stdlib_for_type_checking};
...
pub fn check_file(path: &PathBuf) {
    ...
    write_stdlib_for_type_checking(&dir);
...
/// Scan R files in `r_dir` for `#' @export` annotations and write a NAMESPACE file.
/// This ensures correct exports even when devtools is absent, excluding internal
/// std.R helpers (struct, spread, typr_spread_record, etc.) that carry no @export tag.
fn write_namespace(r_dir: &Path) {
    let mut exports: Vec<String> = Vec::new();
    let mut s3methods: Vec<(String, String)> = Vec::new();
    ...
    for entry in read_dir.flatten() {
        ...
        collect_exports_from_r(&content, &mut exports, &mut s3methods);
    }

    let mut namespace = String::from("# Generated by typr: do not edit by hand\n\n");
    exports.sort();
    exports.dedup();
    for name in &exports {
    ...
    s3methods.sort();
    s3methods.dedup();
    for (generic, class) in &s3methods {
        namespace.push_str(&format!("S3method({},{})\n", generic, class));
    }

    let namespace_path = r_dir.parent().unwrap_or(Path::new(".")).join("NAMESPACE");
    if let Err(e) = fs::write(&namespace_path, namespace) {
        eprintln!("Warning: could not write NAMESPACE: {}", e);
    }
}

/// Parse a single R file's content and accumulate `#' @export` entries.
/// - Plain functions → `exports`
/// - S3 methods (preceded by `#' @method generic Class`) → `s3methods`
fn collect_exports_from_r(
    content: &str,
    exports: &mut Vec<String>,
    s3methods: &mut Vec<(String, String)>,
) {
    let lines: Vec<&str> = content.lines().collect();
    let mut i = 0;
    while i < lines.len() {
        if lines[i].trim() == "#' @export" {
            let mut method_entry: Option<(String, String)> = None;
            let mut j = i + 1;
            while j < lines.len() && lines[j].starts_with("#'") {
                let roxy = lines[j].trim_start_matches("#'").trim();
                if let Some(rest) = roxy.strip_prefix("@method ") {
                    let parts: Vec<&str> = rest.splitn(2, ' ').collect();
                    if parts.len() == 2 {
                        method_entry = Some((parts[0].to_string(), parts[1].trim().to_string()));
                    }
                }
                j += 1;
            }
            if j < lines.len() {
                if let Some(pos) = lines[j].find(" <- function") {
                    let name = lines[j][..pos].trim().trim_matches('`');
                    if !name.is_empty() {
                        if let Some((generic, class)) = method_entry {
                            s3methods.push((generic, class));
                        } else {
...
fn build_project_impl(test_mode: bool, quiet: bool, skip_document: bool) {
    ...
    write_loader(&dir);
    write_namespace(&PathBuf::from("R"));
    step.done();
...
impl FileData {
    fn get_file_content(path: &Path) -> FileData {
        let raw_code = fs::read_to_string(path).unwrap_or_else(|e| {
            eprintln!("Cannot read {:?}: {}", path, e);
            std::process::exit(1);
        });
        let content = strip_shebang(&raw_code);
        let file_name = path.to_str().unwrap().to_string();
        Self::new_file_data(content, &file_name)
    }

    fn new_file_data(file_name: &str, content: &str) -> FileData {
        FileData {
...
fn run_file_impl(path: &Path, keep_files: bool, profile: bool) {
    let file_data = FileData::get_file_content(path);

    let step = Step::new("Parsing");
    let lang = parse_code_from_str(
        &file_data.content,
        &file_data.file_name,
        Environment::StandAlone,
    );
    ...
    let working_directory = get_working_directory(keep_files);

    write_stdlib_for_type_checking(&working_directory);
    let context = Context::default();
    ...
    // Change CWD to working_directory before transpiling so that write_output_file
    // (used for external module .R files) writes relative to working_directory,
    ...
    let r_path = working_directory.join(&r_file_name);
    let step = Step::new("Running");

    //Settings for profiling
    let result = if profile {
        let profile_script = get_profile_script(&r_path);
        Command::new("Rscript")
...
fn get_profile_script(r_path: &PathBuf) -> String {
...
fn get_working_directory(keep_files: bool) -> PathBuf {
    let working_directory = if keep_files {
        PathBuf::from(".")
    } else {
